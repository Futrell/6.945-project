...
(define (make-simple-language parent-agent) 
  ...
  (define grammar-func (lambda () grammar))
  ...
  (make-language grammar-func 
                 dont-know-message-for-meaning
                 dont-know-meaning-of-message
                 update-grammar!
                 parent-agent))

(define (make-language grammar-func 
                       dont-know-message-for-meaning
                       dont-know-meaning-of-message
                       update-grammar!
                       parent-agent)
  ...
  (define (meaning->message meaning parameters)
    (let ((message
           (lookup-ref meaning (grammar-func) 0))
          (agent-to-speak (car parameters))); grammar-lookup 
  ...
  (define (message->meaning message parameters)
    (let ((meaning 
           (lookup-ref message (grammar-func) 1))
